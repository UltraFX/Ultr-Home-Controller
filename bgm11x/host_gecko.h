...
/** 
                ...
                {p}{b}NOTE:{/b} Soft timer is halted also. Hardware interrupt is the only way to wake up from power mode 2 when radio is halted.{/p}
             **/
...
/** This command can be used to set the maximum size of ATT Message Transfer Units (MTU). If maximum ATT MTU is set to larger than 23, MTU is exchanged automatically after Bluetooth LE connection has been established. **/
...
/** This command must be used to send a response to a {a href="#evt_gatt_server_user_read_request"}user_read_request{/a} event. The response needs to be sent within 30 second, otherwise no more GATT 
            ...
            Only GATT MTU - 1 amount of data can be sent once. Client will continue reading by sending new read request with
            increased offset value if it receives MTU - 1 amount of data. **/
...
/** This command can be used to set the local GATT database. **/
...
/** This command can be used to close an endpoint or a BLE connection. This command must always be used to close an endpoint with WAIT_CLOSE status. This is to free the endpoint for future reuse in the case when the remote side closes the connection. **/
...
static inline struct gecko_msg_test_dtm_tx_rsp_t* gecko_cmd_test_dtm_tx(uint8 packet_type,uint8 length,uint8 channel)
{
    ...
    gecko_cmd_msg->header=gecko_cmd_test_dtm_tx_id+((3)<<8);
    gecko_handle_command(gecko_cmd_msg->header,&gecko_cmd_msg->data.payload);
...
static inline struct gecko_msg_test_dtm_rx_rsp_t* gecko_cmd_test_dtm_rx(uint8 channel)
{
    ...
    gecko_cmd_msg->header=gecko_cmd_test_dtm_rx_id+((1)<<8);
    gecko_handle_command(gecko_cmd_msg->header,&gecko_cmd_msg->data.payload);
...
/** 
                This command can be used to perform suitable actions for HomeKit notifications according to a connection state. When device is in connected state it sends empty indication to controller. When device is disconnected it starts fast advertising with parameters set in {a href="#cmd_homekit_configure"}homekit_configure{/a}, after timeout it reverts previous advertising settings. For both states it sets suitable Global State Number value.
             **/
static inline struct gecko_msg_homekit_event_notification_rsp_t* gecko_cmd_homekit_event_notification(uint8 connection,uint16 characteristic)
{
    ...
    gecko_cmd_msg->header=gecko_cmd_homekit_event_notification_id+((3)<<8);
    gecko_handle_command(gecko_cmd_msg->header,&gecko_cmd_msg->data.payload);
